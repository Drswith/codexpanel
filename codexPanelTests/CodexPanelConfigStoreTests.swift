...
final class CodexPanelConfigStoreTests: CodexPanelTestCase {
    ...
    func testLoadOrMigrateSkipsUnknownProviderKindWithoutLosingOAuthAccounts() throws {
        ...
        let oauthProvider = CodexPanelProvider(
            id: "openai-oauth",
            kind: .openAIOAuth,
            label: "OpenAI",
            enabled: true,
            activeAccountId: account.accountId,
            accounts: [CodexPanelProviderAccount.fromTokenAccount(account, existingID: account.accountId)]
        )
        let config = CodexPanelConfig(
            active: CodexPanelActiveSelection(providerId: oauthProvider.id, accountId: account.accountId),
    ...
    func testSavePersistsOpenRouterProviderInLegacyCompatibleShape() throws {
        ...
        let provider = CodexPanelProvider(
            ...
            cachedModelCatalog: [
                ...
                CodexPanelOpenRouterModel(id: "openai/gpt-4.1", name: "GPT-4.1"),
            ],
    ...
    func testLoadOrMigrateRestoresOpenRouterSwitchModeSelectionFromCompatPersistence() throws {
        ...
        let customAccount = CodexPanelProviderAccount(
        ...
        let customProvider = CodexPanelProvider(
        ...
        try store.save(
            CodexPanelConfig(
                active: CodexPanelActiveSelection(providerId: customProvider.id, accountId: customAccount.id),
                openAI: CodexPanelOpenAISettings(
                    accountUsageMode: .switchAccount,
                    switchModeSelection: CodexPanelActiveSelection(
    ...
    func testLoadOrMigrateDefaultsUnknownOpenAISettingsEnumValuesWithoutLosingOAuthAccounts() throws {
        ...
        let config = CodexPanelConfig(
            ...
            openAI: CodexPanelOpenAISettings(
                ...
                accountUsageMode: .switchAccount,
                switchModeSelection: CodexPanelActiveSelection(providerId: oauthProvider.id, accountId: account.accountId),
                accountOrderingMode: .manual,
                manualActivationBehavior: .launchNewInstance,
    ...
    func testLoadOrMigrateRemapsLegacyOAuthIDsToUserScopedIDs() throws {
        ...
        let legacyStored = CodexPanelProviderAccount(
            id: remoteAccountID,
            kind: .oauthTokens,
        ...
        let config = CodexPanelConfig(
            active: CodexPanelActiveSelection(providerId: provider.id, accountId: remoteAccountID),
            openAI: CodexPanelOpenAISettings(accountOrder: [remoteAccountID]),
            providers: [provider]
        )
        try store.save(config)
    ...
    func testLoadOrMigrateKeepsSharedTeamAccountsWhenAccountUserIDClaimIsMissing() throws {
        ...
        let storedFirst = CodexPanelProviderAccount.fromTokenAccount(first, existingID: "legacy-first")
        let storedSecond = CodexPanelProviderAccount.fromTokenAccount(second, existingID: "legacy-second")
        let provider = CodexPanelProvider(
            id: "openai-oauth",
            kind: .openAIOAuth,
            label: "OpenAI",
        ...
        try self.writeConfig(
            CodexPanelConfig(
                active: CodexPanelActiveSelection(providerId: provider.id, accountId: storedFirst.id),
                openAI: CodexPanelOpenAISettings(accountOrder: [storedFirst.id, storedSecond.id]),
                providers: [provider]
            )
        )
    ...
    func testLoadOrMigrateSanitizesHistoricalOverWindowResetAt() throws {
        ...
        let stored = CodexPanelProviderAccount(
            id: "acct_over_window",
            kind: .oauthTokens,
        ...
        try store.save(
            CodexPanelConfig(
                active: CodexPanelActiveSelection(providerId: provider.id, accountId: stored.id),
                providers: [provider]
            )
        )
    ...
    func testLoadOrMigratePreservesOAuthLifecycleMetadataRoundtrip() throws {
        ...
        let stored = CodexPanelProviderAccount.fromTokenAccount(account, existingID: account.accountId)
        let provider = CodexPanelProvider(
            id: "openai-oauth",
            kind: .openAIOAuth,
            label: "OpenAI",
    ...
    func testLoadOrMigrateAbsorbsNewerAuthJSONSnapshotForSameAccount() throws {
        ...
        var stored = CodexPanelProviderAccount.fromTokenAccount(oldAccount, existingID: oldAccount.accountId)
    ...
    func testLoadOrMigrateKeepsLocalSnapshotWhenAuthJSONIsOlder() throws {
        ...
        let stored = CodexPanelProviderAccount.fromTokenAccount(localAccount, existingID: localAccount.accountId)
        let provider = CodexPanelProvider(
            id: "openai-oauth",
            kind: .openAIOAuth,
            label: "OpenAI",
    ...
    func testLoadOrMigrateDoesNotAbsorbDifferentSharedTeamAccountThatOnlyMatchesRemoteAccountID() throws {
        ...
        var stored = CodexPanelProviderAccount.fromTokenAccount(localAccount, existingID: localAccount.accountId)
        stored.tokenExpired = true
        let provider = CodexPanelProvider(
            id: "openai-oauth",
            kind: .openAIOAuth,
    ...
    private func makeStoredOAuthAccount(
        ...
        return CodexPanelProviderAccount.fromTokenAccount(account, existingID: account.accountId)
    }
    ...
    private func makeOAuthConfig(
        ...
        return CodexPanelConfig(
            ...
            openAI: CodexPanelOpenAISettings(accountOrder: accounts.map(\.id)),
            providers: [provider]
        )
    }
...
private struct LegacyV118Config: Codable {
    ...
    var desktop: CodexPanelDesktopSettings = CodexPanelDesktopSettings()

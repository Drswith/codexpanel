...
final class LocalCostSummaryServiceTests: CodexPanelTestCase {
    ...
    func testUsageTotalTokensTreatsCachedInputAsInputSubset() {
    ...
    func testGPT55DefaultPricingMatchesPerMillionRates() {
        let usage = SessionLogStore.Usage(
            inputTokens: 100,
            cachedInputTokens: 20,
            outputTokens: 20
    ...
    func testGPT54LongContextUsesPremiumButMiniDoesNot() {
        ...
        let gpt54Baseline = LocalCostPricing.costUSD(model: "gpt-5.4", usage: usage)
        let gpt54Premium = LocalCostPricing.costUSD(model: "gpt-5.4", usage: usage, sessionUsage: usage)
        let gpt54MiniBaseline = LocalCostPricing.costUSD(model: "gpt-5.4-mini", usage: usage)
        let gpt54MiniWithLongContext = LocalCostPricing.costUSD(
            model: "gpt-5.4-mini",
            usage: usage,
            sessionUsage: usage
        )

        XCTAssertGreaterThan(gpt54Premium, gpt54Baseline)
        XCTAssertEqual(gpt54MiniWithLongContext, gpt54MiniBaseline, accuracy: 1e-12)
    }
    ...
    func testLoadRefreshesChangedSessionFileInsteadOfServingStaleCache() throws {
        ...
        let cacheURL = home.appendingPathComponent(".codexpanel/test-cost-session-cache.json")
        let ledgerURL = home.appendingPathComponent(".codexpanel/test-cost-event-ledger.json")
    ...
    private func makeStore(
        ...
        SessionLogStore(
            ...
            persistedCacheURL: home.appendingPathComponent(".codexpanel/test-cost-session-cache.json"),
            persistedUsageLedgerURL: home.appendingPathComponent(".codexpanel/test-cost-event-ledger.json"),
    ...
    private func writeLegacyCostSessionCache(
        ...
        try CodexPaths.writeSecureFile(
            data,
            to: home.appendingPathComponent(".codexpanel/test-cost-session-cache.json")
        )
    }
    ...
    private func writePersistedLedger(
        ...
        try CodexPaths.writeSecureFile(
            data,
            to: home.appendingPathComponent(".codexpanel/test-cost-event-ledger.json")
        )
    }

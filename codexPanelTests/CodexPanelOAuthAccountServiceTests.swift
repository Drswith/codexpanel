...
final class CodexPanelOAuthAccountServiceTests: CodexPanelTestCase {
    ...
    func testImportActivatedAccountSynchronizesAuthAndConfig() throws {
        let service = CodexPanelOAuthAccountService()
    ...
    func testImportAccountsUpsertsAndPreservesMetadata() throws {
        ...
        let existingAccount = CodexPanelProviderAccount(
            id: "acct_existing",
            kind: .oauthTokens,
            label: "Pinned Label",
        ...
        try store.save(
            CodexPanelConfig(
                active: CodexPanelActiveSelection(providerId: provider.id, accountId: existingAccount.id),
                providers: [provider]
            )
        )
    ...
    func testImportAccountsKeepsCompatibleProviderActive() throws {
        let configStore = CodexPanelConfigStore()
        ...
        try configStore.save(
            CodexPanelConfig(
                active: CodexPanelActiveSelection(providerId: compatibleProvider.id, accountId: compatibleAccount.id),
    ...
    func testImportAccountsRollsBackCodexPanelConfigWhenSyncFails() throws {
        let configStore = CodexPanelConfigStore()
        ...
        let failingService = CodexPanelOAuthAccountService(

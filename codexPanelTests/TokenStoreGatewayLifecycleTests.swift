...
@MainActor
final class TokenStoreGatewayLifecycleTests: CodexPanelTestCase {
    ...
    func testOpenRouterLeaseRestoreStartsGatewayWhenInactiveProviderStillHasServiceableState() throws {
        ...
        try self.writeConfig(
            CodexPanelConfig(
                active: CodexPanelActiveSelection(providerId: custom.provider.id, accountId: custom.account.id),
    ...
    func testAggregateLeaseKeepsGatewayRunningAfterSwitchModeChange() throws {
        ...
        let storedAccount = CodexPanelProviderAccount.fromTokenAccount(account, existingID: account.accountId)
        let provider = CodexPanelProvider(
            id: "openai-oauth",
            kind: .openAIOAuth,
            label: "OpenAI",
        ...
        let config = CodexPanelConfig(
            active: CodexPanelActiveSelection(providerId: provider.id, accountId: storedAccount.id),
    ...
    func testAggregateModePreservesSwitchSelectionAndRestoresItWhenSwitchingBack() throws {
        ...
        let storedOAuthAccount = CodexPanelProviderAccount.fromTokenAccount(
            oauthAccount,
            existingID: oauthAccount.accountId
        )
        let compatibleAccount = CodexPanelProviderAccount(
            id: "acct-compatible",
            kind: .apiKey,
            label: "compatible",
        ...
        let config = CodexPanelConfig(
            ...
            openAI: CodexPanelOpenAISettings(accountUsageMode: .switchAccount),
    ...
    func testActivateAbsorbsNewerAuthJSONBeforeSynchronizing() throws {
        ...
        let provider = CodexPanelProvider(
            ...
            accounts: [
                CodexPanelProviderAccount.fromTokenAccount(activeOtherAccount, existingID: activeOtherAccount.accountId),
                CodexPanelProviderAccount.fromTokenAccount(localAccount, existingID: localAccount.accountId),
            ]
        )
        try self.writeConfig(
            CodexPanelConfig(
                active: CodexPanelActiveSelection(providerId: provider.id, accountId: activeOtherAccount.accountId),
                providers: [provider]
            )
        )
        try self.writeAuthJSON(
...
private final class OpenRouterGatewayControllerSpy: OpenRouterGatewayControlling {
    var startCount = 0
    var stopCount = 0
    private(set) var lastProvider: CodexPanelProvider?
    ...
    func updateState(provider: CodexPanelProvider?, isActiveProvider: Bool) {
...
private final class RecordingSyncService: CodexSynchronizing {
    ...
    private(set) var lastConfig: CodexPanelConfig?
...
private extension TokenStoreGatewayLifecycleTests {
    func makeOpenRouterAccount(id: String) -> CodexPanelProviderAccount {
    ...
    func makeOpenRouterProvider(account: CodexPanelProviderAccount) -> CodexPanelProvider {
        CodexPanelProvider(
            id: "openrouter",
            kind: .openRouter,
            label: "OpenRouter",
    ...
    func makeCustomProvider() -> (provider: CodexPanelProvider, account: CodexPanelProviderAccount) {
        let account = CodexPanelProviderAccount(
            id: "acct-compatible",
            kind: .apiKey,
            label: "Compatible",

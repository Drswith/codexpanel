...
final class ResponsesChatCompletionsTranslatorTests: XCTestCase {
    ...
    func testRequestPreservesOrFlattensStructuredContentBasedOnQuirks() throws {
        let body: [String: Any] = [
            "input": [
                [
                    "type": "message",
        ...
        let standard = ResponsesChatCompletionsTranslator.chatRequestBody(
            fromResponses: body,
            model: "m",
            quirks: .standard
        )
        let standardMessages = try XCTUnwrap(standard["messages"] as? [[String: Any]])
        let content = try XCTUnwrap(standardMessages.first?["content"] as? [[String: Any]])
        XCTAssertEqual(content.first?["type"] as? String, "text")
        XCTAssertEqual(content.last?["type"] as? String, "image_url")

        let flattened = ResponsesChatCompletionsTranslator.chatRequestBody(
            fromResponses: body,
            model: "glm-4.6",
            quirks: CodexPanelChatQuirks(flattenContent: true)
        )
        let flattenedMessages = try XCTUnwrap(flattened["messages"] as? [[String: Any]])
        XCTAssertEqual(flattenedMessages.first?["content"] as? String, "describe\n[image]")
    }
    ...
    func testStreamConverterEmitsMessageItemBeforeReasoningDelta() throws {
        ...
        let reasoningEvents = converter.consume(chunk: [
            "choices": [["delta": ["reasoning_content": "thinking"]]],
        ])
        XCTAssertEqual(reasoningEvents.first?["type"] as? String, "response.reasoning_text.delta")

        let emptyReasoning = converter.consume(chunk: [
            "choices": [["delta": ["reasoning_content": ""]]],
        ])
        XCTAssertTrue(emptyReasoning.isEmpty)
    }

...
final class OpenAIAccountUsageModeTransitionExecutorTests: XCTestCase {
    ...
    func testSwitchingIntoAggregateIgnoresLaunchClosure() async throws {
        let tracker = UsageModeTransitionEffectTracker(currentMode: .switchAccount)
...
private final class UsageModeTransitionEffectTracker {
    var currentMode: CodexPanelOpenAIAccountUsageMode
    var applyCount = 0
    var rollbackCount = 0
    var launchCount = 0

    init(currentMode: CodexPanelOpenAIAccountUsageMode) {

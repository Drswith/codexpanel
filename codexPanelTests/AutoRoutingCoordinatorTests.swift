...
final class CodexPanelConfigCompatibilityTests: CodexPanelTestCase {
    func testConfigDecodesMissingDesktopWithDefaults() throws {
        ...
        let config = try JSONDecoder().decode(CodexPanelConfig.self, from: data)
    ...
    func testQuotaSortClampsProMultiplierIntoConfiguredRange() {
        let low = CodexPanelOpenAISettings.QuotaSortSettings(proRelativeToPlusMultiplier: 4)
        let high = CodexPanelOpenAISettings.QuotaSortSettings(proRelativeToPlusMultiplier: 31)
    ...
    func testPreferredDisplayAccountOrderOnlyAppliesInManualMode() {
        var settings = CodexPanelOpenAISettings(

...
final class TokenStoreSettingsTests: CodexPanelTestCase {
    ...
    func testAggregateGatewayProxyURLDefaultsToNilWhenLegacyJSONOmitsIt() throws {
        let encoded = try JSONEncoder().encode(CodexPanelConfig())
        var payload = try XCTUnwrap(JSONSerialization.jsonObject(with: encoded) as? [String: Any])
        var openAI = try XCTUnwrap(payload["openAI"] as? [String: Any])
        openAI.removeValue(forKey: "aggregateGatewayProxyURL")
        payload["openAI"] = openAI

        let legacyData = try JSONSerialization.data(withJSONObject: payload)
        let decoded = try JSONDecoder().decode(CodexPanelConfig.self, from: legacyData)
    ...
    func testInitializationRebuildsLocalCostSummaryWhenCacheIsMissing() throws {
        ...
        let sessionStore = SessionLogStore(
            ...
            persistedCacheURL: URL(fileURLWithPath: ProcessInfo.processInfo.environment["CODEXPANEL_HOME"] ?? NSTemporaryDirectory())
                .appendingPathComponent(".codexpanel/test-cost-session-cache.json"),
            persistedUsageLedgerURL: URL(fileURLWithPath: ProcessInfo.processInfo.environment["CODEXPANEL_HOME"] ?? NSTemporaryDirectory())
                .appendingPathComponent(".codexpanel/test-cost-event-ledger.json")
        )
        let store = self.makeTokenStore(
    ...
    func testInitializationSeedsHistoricalModelsFromConfigThenRefreshesInBackground() throws {
        ...
        let sessionStore = SessionLogStore(
            codexRootURL: CodexPaths.codexRoot,
            persistedCacheURL: URL(fileURLWithPath: ProcessInfo.processInfo.environment["CODEXPANEL_HOME"] ?? NSTemporaryDirectory())
                .appendingPathComponent(".codexpanel/test-historical-models-session-cache.json"),
            persistedUsageLedgerURL: URL(fileURLWithPath: ProcessInfo.processInfo.environment["CODEXPANEL_HOME"] ?? NSTemporaryDirectory())
                .appendingPathComponent(".codexpanel/test-historical-models-ledger.json")
        )
    ...
    func testInitializationRebuildsLocalCostSummaryWhenCachedSummaryIsZeroButLedgerExists() throws {
        let root = URL(fileURLWithPath: ProcessInfo.processInfo.environment["CODEXPANEL_HOME"] ?? NSTemporaryDirectory())
        ...
        let sessionStore = SessionLogStore(
            ...
            persistedCacheURL: root.appendingPathComponent(".codexpanel/test-cost-session-cache.json"),
        ...
        let ledgerData = try Data(contentsOf: root.appendingPathComponent(".codexpanel/test-cost-event-ledger.json"))
    ...
    func testLegacyOpenAIAccountSettingsUpdatePreservesAggregateGatewayProxyURLWhenChangingOrderAndMode() throws {
        let store = TokenStore.shared
        store.load()
        store.addOrUpdate(try self.makeOAuthAccount(accountID: "acct_alpha", email: "alpha@example.com"))
        store.addOrUpdate(try self.makeOAuthAccount(accountID: "acct_beta", email: "beta@example.com"))
    ...
    func testOpenRouterPinnedModelsRemainAfterSwitchingCurrentModel() throws {
        ...
        let catalog = [
            ...
            CodexPanelOpenRouterModel(id: "google/gemini-2.5-pro", name: "Gemini 2.5 Pro"),
        ]
...
private final class CodexSyncServiceNoOp: CodexSynchronizing {
    func synchronize(config _: CodexPanelConfig) throws {}
}
...
private final class OpenRouterGatewayControllerStub: OpenRouterGatewayControlling {
    func startIfNeeded() {}
    func stop() {}
    func updateState(provider _: CodexPanelProvider?, isActiveProvider _: Bool) {}
}

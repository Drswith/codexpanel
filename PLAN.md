...
- `SettingsWindowView` 增加初始页面参数，支持 `accounts | records | usage | diagnostics | about`，并兼容旧的 `updates` 路由映射到 `about`。

- 新增 CLI 命令接口。
  ```sh
  codexpanel view open settings --page accounts|records|usage|diagnostics|about [--wait 3]

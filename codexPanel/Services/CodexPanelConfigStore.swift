...
final class CodexPanelConfigStore {
    ...
    init(
        ...
        recentOpenRouterModelResolver: @escaping () -> String? = {
            CodexPanelConfigStore.defaultRecentOpenRouterModelIdentifier()
        }
    ) {
    ...
    func loadOrMigrate() throws -> CodexPanelConfig {
        try CodexPaths.ensureDirectories()
        let loaded: CodexPanelConfig
    ...
    func load() throws -> CodexPanelConfig {
        let data = try Data(contentsOf: CodexPaths.barConfigURL)
        return try self.decoder.decode(CodexPanelConfig.self, from: data)
    }

    func save(_ config: CodexPanelConfig) throws {
    ...
    private func migrateFromLegacy() throws -> CodexPanelConfig {
    ...
    private func makeOAuthProvider(auth: [String: Any]) -> CodexPanelProvider? {
        var importedAccounts: [CodexPanelProviderAccount] = []
        ...
        if let data = try? Data(contentsOf: CodexPaths.tokenPoolURL),
           let pool = try? self.decoder.decode(TokenPool.self, from: data) {
            importedAccounts = pool.accounts.map { account in
                CodexPanelProviderAccount.fromTokenAccount(account, existingID: account.accountId)
            }
        }
    ...
    private func makeImportedProviderIfNeeded(
        baseURL: String?,
        apiKey: String,
        existingProviders: [CodexPanelProvider]
    ) -> CodexPanelProvider? {
    ...
    private func resolveActiveSelection(
    ...
    ) -> CodexPanelActiveSelection {
        if let baseURL = toml.openAIBaseURL,
           let provider = providers.first(where: { $0.baseURL == baseURL }) {
            return CodexPanelActiveSelection(providerId: provider.id, accountId: provider.activeAccount?.id)
        }
        ...
        if let snapshot = self.authJSONSnapshot(from: auth),
           let provider = providers.first(where: { $0.kind == .openAIOAuth }) {
            ...
            return CodexPanelActiveSelection(providerId: provider.id, accountId: selected?.id)
        }
        ...
        return CodexPanelActiveSelection(providerId: fallbackProvider?.id, accountId: fallbackProvider?.activeAccount?.id)
    }

    private func normalizeOAuthAccountIdentities(
        in original: CodexPanelConfig
    ) -> (config: CodexPanelConfig, migratedAccountIDs: [String: String], changed: Bool) {
        var config = original
        ...
        var migratedAccounts: [CodexPanelProviderAccount] = []
        var changed = false
    ...
    private func normalizeOpenRouterProviders(
        in original: CodexPanelConfig
    ) -> (config: CodexPanelConfig, changed: Bool) {
        var config = original
        ...
        var mergedProvider = matchingProviders.first(where: { $0.kind == .openRouter }) ?? CodexPanelProvider(
            id: "openrouter",
            kind: .openRouter,
            label: "OpenRouter",
            enabled: true
        ...
        var mergedAccounts: [CodexPanelProviderAccount] = []
        ...
        for provider in matchingProviders {
            ...
            mergedProvider.pinnedModelIDs = CodexPanelProvider.resolvedPinnedModelIDs(
        ...
        if let switchModeSelection = config.openAI.switchModeSelection,
           ...
           matchingIDs.contains(switchProviderID) {
            ...
            let normalizedSelection = CodexPanelActiveSelection(
    ...
    private func legacyCompatiblePersistenceConfig(from original: CodexPanelConfig) -> CodexPanelConfig {
        var config = original
    ...
    private func mergeOAuthAccount(
        ...
        incoming: CodexPanelProviderAccount
        ...
        if replacesProfile == false {
            merged.username = existing.username ?? incoming.username
            merged.displayName = existing.displayName ?? incoming.displayName
    ...
    private func uniqueOAuthAccount(
        in provider: CodexPanelProvider,
    ...
    private func accountFromAuthSnapshot(_ snapshot: OpenAIAuthJSONSnapshot) -> CodexPanelProviderAccount {
        var stored = CodexPanelProviderAccount.fromTokenAccount(
            snapshot.account,
    ...
    private func matchingStoredAccountIndex(
        in accounts: [CodexPanelProviderAccount],
        snapshot: OpenAIAuthJSONSnapshot,
        onlyAccountIDs: Set<String>?
    ) -> Int? {
        let eligibleAccounts: [(offset: Int, element: CodexPanelProviderAccount)] = accounts.enumerated().filter { pair in
    ...
    private func shouldAbsorbAuthSnapshot(
        ...
        into stored: CodexPanelProviderAccount
    ) -> Bool {
    ...
    private func isLegacyOpenRouterProvider(_ provider: CodexPanelProvider) -> Bool {
    ...
    private func openRouterAccountDeduplicationKey(for account: CodexPanelProviderAccount) -> String {

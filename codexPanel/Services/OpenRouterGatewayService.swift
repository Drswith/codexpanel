...
final class OpenRouterGatewayService: OpenRouterGatewayControlling {
    nonisolated static let mockRequestBodyPropertyKey = "codexpanel.mockOpenRouterRequestBody"
    ...
    private let listenerQueue = DispatchQueue(label: "com.codexpanel.openrouter-gateway.listener")
    private let stateQueue = DispatchQueue(label: "com.codexpanel.openrouter-gateway.state")
    ...
    private func receiveRequest(on connection: NWConnection, accumulated: Data) {
        connection.receive(minimumIncompleteLength: 1, maximumLength: 131_072) { [weak self] data, _, isComplete, error in
            ...
            if let error {
                NSLog("codexpanel OpenRouter gateway receive failed: %@", error.localizedDescription)
                connection.cancel()
                return
            }
    ...
    private func forwardResponsesRequest(_ request: ParsedGatewayRequest, on connection: NWConnection) async {
        do {
            ...
            self.sendJSONResponse(
                on: connection,
                statusCode: 502,
                body: #"{"error":{"message":"codexpanel OpenRouter gateway failed to reach upstream"}}"#
            )
        }
    }
            ...
            if let role = (message["role"] as? String)?.lowercased(),
               role == "assistant" {
                ...
                if (message["id"] as? String)?.isEmpty != false {
                    message["id"] = "msg_codexpanel_\(index)"
                }
            }

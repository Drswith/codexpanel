...
enum ChatCompletionsGatewayConfiguration {
    ...
    static var port: UInt16 {
        CodexPanelRuntimeProfile.current.network.chatCompletionsGatewayPort
    }

    static var baseURLString: String {
        CodexPanelRuntimeProfile.current.network.chatCompletionsGatewayBaseURLString
    }
}
...
final class ChatCompletionsGatewayService: ChatCompletionsGatewayControlling {
    nonisolated static let mockRequestBodyPropertyKey = "codexpanel.mockChatCompletionsRequestBody"

    private let listenerQueue = DispatchQueue(label: "com.codexpanel.chat-completions-gateway.listener")
    private let stateQueue = DispatchQueue(label: "com.codexpanel.chat-completions-gateway.state")
    private let urlSession: URLSession
    private let runtimeConfiguration: ChatCompletionsGatewayRuntimeConfiguration

    private var listener: NWListener?
    private var provider: CodexPanelProvider?
    ...
    func startIfNeeded() {
        self.listenerQueue.async {
            ...
            do {
                ...
                NSLog("codexpanel chat completions gateway failed to start: %@", error.localizedDescription)
            }
        }
    }
    ...
    func updateState(provider: CodexPanelProvider?, isActiveProvider _: Bool) {
    ...
    private func receiveRequest(on connection: NWConnection, accumulated: Data) {
        connection.receive(minimumIncompleteLength: 1, maximumLength: 131_072) { [weak self] data, _, isComplete, error in
            guard let self else { return }
            if let error {
                NSLog("codexpanel chat completions gateway receive failed: %@", error.localizedDescription)
                connection.cancel()
                return
            }
    ...
    private func forwardResponsesRequest(_ request: ParsedGatewayRequest, on connection: NWConnection) async {
        do {
            ...
            self.sendJSONResponse(
                on: connection,
                statusCode: 502,
                body: #"{"error":{"message":"codexpanel chat completions gateway failed to reach upstream"}}"#
            )
        }
    }
    ...
    private func forwardModelsRequest(on connection: NWConnection) async {
        do {
            ...
            self.sendJSONResponse(
                on: connection,
                statusCode: 502,
                body: #"{"error":{"message":"codexpanel chat completions gateway failed to list models"}}"#
            )
        }
    }
    ...
    private func currentState() -> ChatCompletionsGatewayState? {
        self.stateQueue.sync {
            ...
            return ChatCompletionsGatewayState(
                ...
                quirks: CodexPanelProviderPresetCatalog.quirks(forPresetID: provider.presetID)
            )
        }
    }

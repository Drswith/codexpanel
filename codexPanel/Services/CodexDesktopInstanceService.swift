...
/// 4. 通过 NSWorkspace 启动，进程由 launchd 托管，不随 Codex Panel 生命周期退出。
@MainActor
final class CodexDesktopInstanceService {
    ...
    init(
        ...
        self.codexHomeURL = codexHomeURL ?? CodexPaths.codexRoot
        self.instancesRootURL = instancesRootURL ?? CodexPaths.managedCodexDesktopProfilesURL
        self.registryURL = registryURL ?? CodexPaths.managedInstancesRegistryURL
        self.now = now
    }

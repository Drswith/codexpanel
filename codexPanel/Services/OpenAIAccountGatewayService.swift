...
extension Notification.Name {
    static let openAIAccountGatewayDidRouteAccount = Notification.Name(
        "com.codexpanel.openai-gateway.did-route-account"
    )
}
...
enum OpenAIAccountGatewayConfiguration {
    // 固定使用 IPv4 环回地址，避免 localhost 在系统代理或地址族选择异常时被误送入代理。
...
struct OpenAIAccountGatewayRuntimeConfiguration {
    ...
    init(
        host: String,
        port: UInt16,
        upstreamResponsesURL: URL,
        upstreamResponsesCompactURL: URL,
        upstreamImagesGenerationsURL: URL = OpenAIAccountGatewayConfiguration.upstreamImagesGenerationsURL,
        upstreamImagesEditsURL: URL = OpenAIAccountGatewayConfiguration.upstreamImagesEditsURL
    ) {
        self.host = host
        self.port = port
        self.upstreamResponsesURL = upstreamResponsesURL
        self.upstreamResponsesCompactURL = upstreamResponsesCompactURL
        self.upstreamImagesGenerationsURL = upstreamImagesGenerationsURL
        self.upstreamImagesEditsURL = upstreamImagesEditsURL
    }
...
struct OpenAIAccountGatewaySystemProxySnapshot: Equatable {
    ...
    nonisolated static var disabledConnectionProxyDictionary: [AnyHashable: Any] {
        [
...
nonisolated struct OpenAIAccountGatewayConfiguredProxy: Hashable {
    ...
    init?(address: String?) {
        guard let rawAddress = address?.trimmingCharacters(in: .whitespacesAndNewlines),
              rawAddress.isEmpty == false,
              let components = URLComponents(string: rawAddress),
        ...
        guard Self.isDisabledInteropProfile(object) == false,
              ...
              let scheme = Self.normalizedScheme(
                Self.trimmedString(object["protocol"])
                    ...
                    ?? proxyKey.flatMap({ Self.proxyKeyPart($0, index: 0) })
              ),
    ...
    static func profilesByKey(
        fromInteropProxiesJSON json: String?
    ) -> [String: OpenAIAccountGatewayConfiguredProxy] {
        guard let json,
              let data = json.data(using: .utf8),
              let object = try? JSONSerialization.jsonObject(with: data),
              let profiles = object as? [[String: Any]] else {
            return [:]
        }

        var result: [String: OpenAIAccountGatewayConfiguredProxy] = [:]
        for profile in profiles {
            guard let proxyKey = Self.trimmedString(profile["proxy_key"]),
                  let configuredProxy = Self(interopObject: profile) else {
                continue
            }
            result[proxyKey] = configuredProxy
        }
        return result
    }
    ...
    private init?(scheme: String, host: String, port: Int) {
        ...
        self.address = "\(scheme)://\(Self.addressHost(normalizedHost)):\(port)"
        self.kind = kind
    ...
    private static func normalizedScheme(_ value: String?) -> String? {
        guard let value = value?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased(),
              self.kind(for: value) != nil else {
            return nil
        }
        return value
    }

    private static func kind(for scheme: String) -> Kind? {
        switch scheme {
        case "http", "https":
            return .http
        case "socks", "socks5":
    ...
    private static func proxyKeyPart(_ proxyKey: String, index: Int) -> String? {
        ...
        return Self.trimmedString(parts[index])
    }

    private static func hasNoCredentials(in object: [String: Any], proxyKey: String?) -> Bool {
        let explicitCredentials = ["username", "user", "password"]
            .compactMap { Self.trimmedString(object[$0]) }
        guard explicitCredentials.isEmpty else { return false }
...
func profilesByKey(
    ...
    OpenAIAccountGatewayConfiguredProxy.profilesByKey(fromInteropProxiesJSON: json)
}
...
private struct OpenAIAccountGatewaySnapshot {
    ...
    var quotaSortSettings: CodexPanelOpenAISettings.QuotaSortSettings
...
final class OpenAIAccountGatewayService: OpenAIAccountGatewayControlling {
    ...
    nonisolated static let mockRequestBodyPropertyKey = "codexpanel.mockRequestBody"

    private let listenerQueue = DispatchQueue(label: "com.codexpanel.openai-gateway.listener")
    private let stateQueue = DispatchQueue(label: "com.codexpanel.openai-gateway.state")
    private let explicitProxySessionQueue = DispatchQueue(label: "com.codexpanel.openai-gateway.explicit-proxy-session")
    ...
    private var quotaSortSettings = CodexPanelOpenAISettings.QuotaSortSettings()
    private var accountUsageMode: CodexPanelOpenAIAccountUsageMode = .switchAccount
    ...
    nonisolated private static func liveDiagnosticsReporter(
        ...
        NSLog(
            "codexpanel OpenAI gateway upstream failure route=%@ failureClass=%@ status=%@ errorDomain=%@ errorCode=%@ loopbackProxySafe=%@",
            diagnostic.route,
    ...
    private func receiveRequest(on connection: NWConnection, accumulated: Data) {
        connection.receive(minimumIncompleteLength: 1, maximumLength: 131_072) { [weak self] data, _, isComplete, error in
            ...
            if let error {
                NSLog("codexpanel OpenAI gateway receive failed: %@", error.localizedDescription)
                connection.cancel()
                return
            }
        ...
        _ = await self.routePOSTResponsesCandidates(
            ...
            onSyntheticGatewayFailure: {
                self.sendJSONResponse(
                    on: connection,
                    statusCode: 502,
                    body: #"{"error":{"message":"codexpanel gateway failed to reach OpenAI upstream"}}"#
                )
            }
        ...
        case .compact:
            ...
            if upstreamRequest.value(forHTTPHeaderField: "version") == nil {
                upstreamRequest.setValue(
                    OpenAIAccountGatewayConfiguration.defaultCodexCLIVersion,
    ...
    private func upstreamSession(for account: TokenAccount) -> URLSession {
        self.upstreamSession(for: account, route: .responses)
    }

    private func configuredProxy(
    ...
    private func pruneExplicitProxySessions(
        keeping proxies: Set<OpenAIAccountGatewayConfiguredProxy>
    ) {
        ...
        connection.receive(minimumIncompleteLength: 1, maximumLength: 65_536) { [weak self] data, _, isComplete, error in
            ...
            Task {
                if let error {
                    ...
                    self.clearBinding(stickyKey: stickyKey, accountID: accountID)
                    connection.cancel()
                    NSLog("codexpanel websocket receive failed: %@", error.localizedDescription)
                    return
                }
...
extension OpenAIAccountGatewayService {
    ...
    func configuredProxyForTesting(
        accountID: String
    ) -> OpenAIAccountGatewayConfiguredProxy? {
        self.configuredProxy(forAccountID: accountID)
    }

    func usesSameExplicitProxySessionForHTTPAndWebSocketForTesting(
        accountID: String
    ) -> Bool {
        guard let account = self.snapshot().accounts.first(where: { $0.accountId == accountID }),
              self.configuredProxy(forAccountID: accountID) != nil else {
            return false
        }

        let httpSession = self.upstreamSession(for: account)
        let webSocketSession = self.upstreamSession(for: account)
        return httpSession === webSocketSession
    }
        ...
        return await self.routePOSTResponsesCandidates(
            ...
            onSyntheticGatewayFailure: {
                OpenAIAccountGatewayTestResponse(
                    statusCode: 502,
                    headers: ["Content-Type": "application/json"],
                    body: Data(#"{"error":{"message":"codexpanel gateway failed to reach OpenAI upstream"}}"#.utf8)
                )
            }
        ) { _, _, _, _, _ in
            throw failure
    ...
    private func gatewayErrorBody(message: String) -> String {
        ...
        guard let data = try? JSONSerialization.data(withJSONObject: payload),
            ...
            return #"{"error":{"message":"codexpanel gateway failed to reach OpenAI upstream"}}"#
        }
        return body
    }

...
enum CodexPanelProviderKind: String, Codable {
    case openAIOAuth = "openai_oauth"
    case openAICompatible = "openai_compatible"
    case openRouter = "openrouter"
}

enum CodexPanelWireAPI: String, Codable, CaseIterable, Identifiable {
    case responses
    case chat
...
enum CodexPanelUsageDisplayMode: String, Codable, CaseIterable, Identifiable {
    case remaining
    case used
...
enum CodexPanelAccountKind: String, Codable {
...
struct CodexPanelActiveSelection: Codable, Equatable {
    var providerId: String?
    var accountId: String?
}

struct CodexPanelDesktopSettings: Codable, Equatable {
...
struct CodexPanelModelPricing: Codable, Equatable {
    ...
    static let zero = CodexPanelModelPricing(
        inputUSDPerToken: 0,
...
enum CodexPanelOpenAIManualActivationBehavior: String, Codable, CaseIterable, Identifiable {
...
enum CodexPanelOpenAIAccountUsageMode: String, Codable, CaseIterable, Identifiable {
...
enum CodexPanelOpenAIAccountOrderingMode: String, Codable, CaseIterable, Identifiable {
    case quotaSort
    case manual
...
struct CodexPanelOpenAISettings: Codable, Equatable {
    ...
    var switchModeSelection: CodexPanelActiveSelection?
    ...
    init(
        accountOrder: [String] = [],
        accountUsageMode: CodexPanelOpenAIAccountUsageMode = .switchAccount,
        switchModeSelection: CodexPanelActiveSelection? = nil,
...
struct CodexPanelProviderAccount: Codable, Identifiable, Equatable {
    var id: String
    var kind: CodexPanelAccountKind
    var label: String
    ...
    func sanitizedQuotaSnapshot(now: Date = Date()) -> CodexPanelProviderAccount {
    ...
    static func fromTokenAccount(_ account: TokenAccount, existingID: String? = nil) -> CodexPanelProviderAccount {
        let normalizedAccount = account.normalizedQuotaSnapshot()
        return CodexPanelProviderAccount(
...
struct CodexPanelProvider: Codable, Identifiable, Equatable {
    var id: String
    var kind: CodexPanelProviderKind
    var label: String
    var enabled: Bool
    var baseURL: String?
    var wireAPI: CodexPanelWireAPI
    var presetID: String?
    var defaultModel: String?
    var selectedModelID: String?
    var pinnedModelIDs: [String]
    var cachedModelCatalog: [CodexPanelOpenRouterModel]
    var modelCatalogFetchedAt: Date?
    var activeAccountId: String?
    var accounts: [CodexPanelProviderAccount]

    init(
        id: String,
        kind: CodexPanelProviderKind,
        label: String,
        enabled: Bool = true,
        baseURL: String? = nil,
        wireAPI: CodexPanelWireAPI = .responses,
        presetID: String? = nil,
        ...
        accounts: [CodexPanelProviderAccount] = []
    ) {
    ...
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        let decodedKind = try container.decode(CodexPanelProviderKind.self, forKey: .kind)
        ...
        self.wireAPI = decodedKind == .openAICompatible
            ? try container.decodeLossyStringEnum(CodexPanelWireAPI.self, forKey: .wireAPI, default: .responses)
            : .responses
        ...
        self.cachedModelCatalog = try container.decodeIfPresent([CodexPanelOpenRouterModel].self, forKey: .cachedModelCatalog) ?? []
        ...
        self.accounts = (try container.decodeIfPresent(
            [FailableDecodable<CodexPanelProviderAccount>].self,
            forKey: .accounts
        ) ?? []).compactMap(\.value)
    }

    var activeAccount: CodexPanelProviderAccount? {
    ...
    var chatCompletionsServiceableSelection: (account: CodexPanelProviderAccount, modelID: String, baseURL: String)? {
    ...
    var openRouterServiceableSelection: (account: CodexPanelProviderAccount, modelID: String)? {
...
struct CodexPanelConfig: Codable {
    var version: Int
    var global: CodexPanelGlobalSettings
    var active: CodexPanelActiveSelection
    var desktop: CodexPanelDesktopSettings
    var modelPricing: [String: CodexPanelModelPricing]
    var openAI: CodexPanelOpenAISettings
    var providers: [CodexPanelProvider]

    init(
        version: Int = 1,
        global: CodexPanelGlobalSettings = CodexPanelGlobalSettings(),
        active: CodexPanelActiveSelection = CodexPanelActiveSelection(),
        desktop: CodexPanelDesktopSettings = CodexPanelDesktopSettings(),
        modelPricing: [String: CodexPanelModelPricing] = [:],
        openAI: CodexPanelOpenAISettings = CodexPanelOpenAISettings(),
        providers: [CodexPanelProvider] = []
    ) {
    ...
    init(from decoder: Decoder) throws {
        ...
        self.global = try container.decodeIfPresent(CodexPanelGlobalSettings.self, forKey: .global) ?? CodexPanelGlobalSettings()
        self.active = try container.decodeIfPresent(CodexPanelActiveSelection.self, forKey: .active) ?? CodexPanelActiveSelection()
        self.desktop = try container.decodeIfPresent(CodexPanelDesktopSettings.self, forKey: .desktop) ?? CodexPanelDesktopSettings()
        self.modelPricing = try container.decodeIfPresent([String: CodexPanelModelPricing].self, forKey: .modelPricing) ?? [:]
        self.openAI = try container.decodeIfPresent(CodexPanelOpenAISettings.self, forKey: .openAI) ?? CodexPanelOpenAISettings()
        self.providers = (try container.decodeIfPresent(
            [FailableDecodable<CodexPanelProvider>].self,
            forKey: .providers
        ) ?? []).compactMap(\.value)
    }

    func provider(id: String?) -> CodexPanelProvider? {
        guard let id else { return nil }
        return self.providers.first(where: { $0.id == id })
    }

    func activeProvider() -> CodexPanelProvider? {
        self.provider(id: self.active.providerId)
    }

    func activeAccount() -> CodexPanelProviderAccount? {
        self.activeProvider()?.accounts.first(where: { $0.id == self.active.accountId }) ?? self.activeProvider()?.activeAccount
    }

    func oauthProvider() -> CodexPanelProvider? {
        self.providers.first(where: { $0.kind == .openAIOAuth })
    }

    func openRouterProvider() -> CodexPanelProvider? {
...
extension CodexPanelConfig {
    mutating func upsertOAuthAccount(_ account: TokenAccount, activate: Bool) -> (storedAccount: CodexPanelProviderAccount, syncCodex: Bool) {
        ...
        if let index = provider.accounts.firstIndex(where: { $0.id == account.accountId }) {
            ...
            let created = CodexPanelProviderAccount.fromTokenAccount(account, existingID: account.accountId)
        ...
        let storedAccount = self.oauthProvider()?.accounts.first(where: { $0.id == storedAccountID })
            ...
            ?? CodexPanelProviderAccount.fromTokenAccount(account, existingID: storedAccountID)
    ...
    mutating func activateOAuthAccount(accountID: String) throws -> CodexPanelProviderAccount {
    ...
    mutating func upsertOpenRouterProvider(
        ...
        apiKey: String,
        activate: Bool
    ) throws -> CodexPanelProviderAccount {
    ...
    mutating func activateOpenRouterAccount(accountID: String) throws -> CodexPanelProviderAccount {
    ...
    mutating func setOpenRouterSelectedModel(_ value: String?) throws {
        ...
        provider.selectedModelID = CodexPanelProvider.normalizedOpenRouterModelID(value)
        provider.pinnedModelIDs = CodexPanelProvider.resolvedPinnedModelIDs(
    ...
    mutating func setOpenRouterModelSelection(
        selectedModelID: String?,
        pinnedModelIDs: [String],
        cachedModelCatalog: [CodexPanelOpenRouterModel]? = nil,
        fetchedAt: Date? = nil
    ) throws {
        guard var provider = self.openRouterProvider() else {
            throw TokenStoreError.providerNotFound
        ...
        let normalizedSelectedModelID = CodexPanelProvider.normalizedOpenRouterModelID(selectedModelID)
    ...
    mutating func updateOpenRouterModelCatalog(
        _ models: [CodexPanelOpenRouterModel],
        fetchedAt: Date
    ) throws {
        guard var provider = self.openRouterProvider() else {
            throw TokenStoreError.providerNotFound
    ...
    mutating func setOpenAIManualActivationBehavior(_ behavior: CodexPanelOpenAIManualActivationBehavior) {
        self.openAI.manualActivationBehavior = behavior
    }

    mutating func setOpenAIAccountUsageMode(_ mode: CodexPanelOpenAIAccountUsageMode) {
    ...
    mutating func captureSwitchModeSelection() {
        ...
        self.openAI.switchModeSelection = CodexPanelActiveSelection(
    ...
    mutating func setOpenAIAccountOrderingMode(_ mode: CodexPanelOpenAIAccountOrderingMode) {
    ...
    private mutating func ensureOAuthProvider() -> CodexPanelProvider {
    ...
    private mutating func ensureOpenRouterProvider() -> CodexPanelProvider {
    ...
    private mutating func upsertProvider(_ provider: CodexPanelProvider) {
    ...
    private func oauthStoredAccount(in provider: CodexPanelProvider, matching accountID: String) -> CodexPanelProviderAccount? {
    ...
    static func canMatchOAuthAccountsByRemoteID(
        _ stored: CodexPanelProviderAccount,
    ...
    private func oauthCredentialsChanged(
        existing: CodexPanelProviderAccount?,
        updated: CodexPanelProviderAccount
    ) -> Bool {
    ...
    private static func uniqueOpenRouterModelCatalog(
        _ models: [CodexPanelOpenRouterModel]
    ) -> [CodexPanelOpenRouterModel] {
        var seen: Set<String> = []
        return models.compactMap { model in
            guard let normalizedID = CodexPanelProvider.normalizedOpenRouterModelID(model.id),
                  seen.insert(normalizedID).inserted else {
                return nil
            }
            return CodexPanelOpenRouterModel(id: normalizedID, name: model.name)
        }
    }

    private static func isSharedOpenAITeamAccount(_ account: CodexPanelProviderAccount) -> Bool {
    ...
    private static func normalizedSharedOpenAIAccountID(
        for account: CodexPanelProviderAccount
    ) -> String? {
